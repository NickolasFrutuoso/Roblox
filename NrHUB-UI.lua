local MacLib = { 
	Options = {}, 
	Folder = "Maclib", 
	-- Forward declarations keep callbacks created before the config helpers
	-- type-safe; the full implementations replace these later in this file.
	RefreshConfigList = function(self, ...) return {} end,
	SaveConfig = function(self, ...) return false, "not initialized" end,
	LoadConfig = function(self, ...) return false, "not initialized" end,
...
function MacLib:Window(Settings)
	...
	local globalSettingsButtonArrow = Instance.new("TextLabel")
	globalSettingsButtonArrow.Name = "GlobalSettingsButtonArrow"
	globalSettingsButtonArrow.BackgroundTransparency = 1
	globalSettingsButtonArrow.FontFace = Font.new(assets.interFont, Enum.FontWeight.SemiBold, Enum.FontStyle.Normal)
	globalSettingsButtonArrow.Text = "<"
	globalSettingsButtonArrow.TextColor3 = Color3.fromRGB(235, 235, 235)
	globalSettingsButtonArrow.TextTransparency = 0.18
	globalSettingsButtonArrow.TextSize = 18
	globalSettingsButtonArrow.TextXAlignment = Enum.TextXAlignment.Center
	globalSettingsButtonArrow.TextYAlignment = Enum.TextYAlignment.Center
	globalSettingsButtonArrow.Size = UDim2.fromScale(1, 1)
	globalSettingsButtonArrow.Parent = globalSettingsButton
	...
	sidebarGroup.Position = UDim2.fromOffset(0, 60)
	sidebarGroup.Size = UDim2.new(1, 0, 1, -60)
	...
	userInfo.Size = UDim2.new(1, 0, 0, 52)
	...
	informationGroupUIPadding.PaddingBottom = UDim.new(0, 8)
	...
	headshot.InputBegan:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
			if homeButtonCallback then
				homeButtonCallback()
			end
		end
	end)
	headshot:SetAttribute("HeadshotHomeClick", true)
	...
	local snapRange = 20
	local minSidebarWidth = 96
	local maxSidebarWidth = math.min(math.floor(base.AbsoluteSize.X * 0.38), base.AbsoluteSize.X - minSidebarWidth)
  ...
  -- Resize completo da janela
  -- Adicionar logo apÃ³s o bloco do dividerInteract e antes de "local topbar = ..."
  
  local resizeHandleSize = 6 -- px de Ã¡rea sensÃ­vel nas bordas
  local minWindowWidth = 400
  local minWindowHeight = 300
  local maxWindowWidth = workspace.CurrentCamera.ViewportSize.X - 20
  local maxWindowHeight = workspace.CurrentCamera.ViewportSize.Y - 20
  ...
  -- Bordas: Top, Bottom, Left, Right, TopLeft, TopRight, BottomLeft, BottomRight
  local resizeBorders = {}
  local borderDefs = {
      { name = "Right",       pos = UDim2.new(1, -resizeHandleSize/2, 0, resizeHandleSize),   size = UDim2.new(0, resizeHandleSize, 1, -resizeHandleSize*2), cursor = "SizeWE" },
      { name = "Left",        pos = UDim2.new(0, -resizeHandleSize/2, 0, resizeHandleSize),   size = UDim2.new(0, resizeHandleSize, 1, -resizeHandleSize*2), cursor = "SizeWE" },
      { name = "Bottom",      pos = UDim2.new(0, resizeHandleSize, 1, -resizeHandleSize/2),   size = UDim2.new(1, -resizeHandleSize*2, 0, resizeHandleSize), cursor = "SizeNS" },
      { name = "Top",         pos = UDim2.new(0, resizeHandleSize, 0, -resizeHandleSize/2),   size = UDim2.new(1, -resizeHandleSize*2, 0, resizeHandleSize), cursor = "SizeNS" },
      { name = "BottomRight", pos = UDim2.new(1, -resizeHandleSize, 1, -resizeHandleSize),    size = UDim2.new(0, resizeHandleSize*2, 0, resizeHandleSize*2), cursor = "SizeNWSE" },
      { name = "BottomLeft",  pos = UDim2.new(0, -resizeHandleSize, 1, -resizeHandleSize),    size = UDim2.new(0, resizeHandleSize*2, 0, resizeHandleSize*2), cursor = "SizeNESW" },
      { name = "TopRight",    pos = UDim2.new(1, -resizeHandleSize, 0, -resizeHandleSize),    size = UDim2.new(0, resizeHandleSize*2, 0, resizeHandleSize*2), cursor = "SizeNESW" },
      { name = "TopLeft",     pos = UDim2.new(0, -resizeHandleSize, 0, -resizeHandleSize),    size = UDim2.new(0, resizeHandleSize*2, 0, resizeHandleSize*2), cursor = "SizeNWSE" },
  }
  
  for _, def in ipairs(borderDefs) do
      local handle = Instance.new("TextButton")
      handle.Name = "ResizeHandle_" .. def.name
      handle.Text = ""
      handle.BackgroundTransparency = 1
      handle.BorderSizePixel = 0
      handle.Position = def.pos
      ...
      handle.MouseButton1Down:Connect(function()
          resizingWindow = true
          resizeDirection = def.name
          resizeStartMouse = UserInputService:GetMouseLocation()
          resizeStartSize = Vector2.new(base.AbsoluteSize.X, base.AbsoluteSize.Y)
          resizeStartPos = Vector2.new(base.Position.X.Offset, base.Position.Y.Offset)
      end)
  
      resizeBorders[def.name] = handle
  end
  ...
  UserInputService.InputChanged:Connect(function(input)
      if not resizingWindow or not resizeDirection then return end
      if input.UserInputType ~= Enum.UserInputType.MouseMovement then return end
  
      local mouse = UserInputService:GetMouseLocation()
      local dx = mouse.X - resizeStartMouse.X
      local dy = mouse.Y - resizeStartMouse.Y
      ...
      local dir = resizeDirection
  
      -- Largura / posiÃ§Ã£o X
      if dir == "Right" or dir == "BottomRight" or dir == "TopRight" then
          newW = math.clamp(resizeStartSize.X + dx, minWindowWidth, maxWindowWidth)
      elseif dir == "Left" or dir == "BottomLeft" or dir == "TopLeft" then
          local clampedW = math.clamp(resizeStartSize.X - dx, minWindowWidth, maxWindowWidth)
          newX = resizeStartPos.X + (resizeStartSize.X - clampedW)
          newW = clampedW
      end
  
      -- Altura / posiÃ§Ã£o Y
      if dir == "Bottom" or dir == "BottomRight" or dir == "BottomLeft" then
          newH = math.clamp(resizeStartSize.Y + dy, minWindowHeight, maxWindowHeight)
      elseif dir == "Top" or dir == "TopRight" or dir == "TopLeft" then
          local clampedH = math.clamp(resizeStartSize.Y - dy, minWindowHeight, maxWindowHeight)
          newY = resizeStartPos.Y + (resizeStartSize.Y - clampedH)
          newH = clampedH
      end
  
      -- Aplica tamanho e posiÃ§Ã£o
      base.Size = UDim2.fromOffset(newW, newH)
      base.Position = UDim2.new(0.5, newX, 0.5, newY)
  
      -- Atualiza content para acompanhar a sidebar
      local currentSidebarWidth = sidebar.AbsoluteSize.X
      content.Size = UDim2.new(0, newW - currentSidebarWidth, 1, 0)
  
      -- Atualiza limites do resize da sidebar
      maxSidebarWidth = newW - minSidebarWidth
  end)
  
  -- Atualizar content quando a janela mudar de tamanho por qualquer meio
  base:GetPropertyChangedSignal("AbsoluteSize"):Connect(function()
      local currentSidebarWidth = sidebar.AbsoluteSize.X
      content.Size = UDim2.new(0, base.AbsoluteSize.X - currentSidebarWidth, 1, 0)
	...
	moveIcon.Position = UDim2.new(1, -92, 0.5, 0)
	...
	local windowActionBar = Instance.new("Frame")
	windowActionBar.Name = "WindowActionBar"
	windowActionBar.AnchorPoint = Vector2.new(1, 0.5)
	windowActionBar.BackgroundTransparency = 1
	windowActionBar.BorderSizePixel = 0
	windowActionBar.Position = UDim2.new(1, 0, 0.5, 0)
	windowActionBar.Size = UDim2.fromOffset(76, 28)
	windowActionBar.Parent = elements

	local windowActionLayout = Instance.new("UIListLayout")
	windowActionLayout.FillDirection = Enum.FillDirection.Horizontal
	windowActionLayout.HorizontalAlignment = Enum.HorizontalAlignment.Right
	windowActionLayout.VerticalAlignment = Enum.VerticalAlignment.Center
	windowActionLayout.Padding = UDim.new(0, 10)
	windowActionLayout.SortOrder = Enum.SortOrder.LayoutOrder
	windowActionLayout.Parent = windowActionBar

	local function NewWindowActionButton(name, order)
		...
		button.Size = UDim2.fromOffset(18, 24)
		button.Parent = windowActionBar

		local iconColor = Color3.fromRGB(218, 218, 218)
		local iconParts = {}
		local function addLine(partName, size, position, rotation)
			local line = Instance.new("Frame")
			line.Name = partName
			line.AnchorPoint = Vector2.new(0.5, 0.5)
			line.BackgroundColor3 = iconColor
			...
			line.Parent = button
			table.insert(iconParts, line)
			return line
		end

		if name == "TopbarMinimize" then
			addLine("MinimizeLine", UDim2.fromOffset(10, 1), UDim2.fromScale(0.5, 0.56), 0)
		elseif name == "TopbarMaximize" then
			local box = Instance.new("Frame")
			box.Name = "MaximizeBox"
			...
			box.Parent = button
			local stroke = Instance.new("UIStroke")
			stroke.Name = "MaximizeStroke"
			stroke.Color = iconColor
			stroke.Transparency = 0.22
			stroke.Thickness = 1
			stroke.Parent = box
			table.insert(iconParts, stroke)
		elseif name == "TopbarClose" then
			addLine("CloseLineA", UDim2.fromOffset(12, 1), UDim2.fromScale(0.5, 0.5), 45)
			addLine("CloseLineB", UDim2.fromOffset(12, 1), UDim2.fromScale(0.5, 0.5), -45)
		end

		local function setIconTransparency(value)
			for _, part in ipairs(iconParts) do
				if part:IsA("UIStroke") then
					Tween(part, TweenInfo.new(0.15, Enum.EasingStyle.Sine), { Transparency = value }):Play()
				else
					Tween(part, TweenInfo.new(0.15, Enum.EasingStyle.Sine), { BackgroundTransparency = value }):Play()
				end
			end
		end
		button.MouseEnter:Connect(function() setIconTransparency(0) end)
		button.MouseLeave:Connect(function() setIconTransparency(0.22) end)
		return button
	end

	local topbarMinimize = NewWindowActionButton("TopbarMinimize", 1)
	local topbarMaximize = NewWindowActionButton("TopbarMaximize", 2)
	local topbarClose = NewWindowActionButton("TopbarClose", 3)
	...
	-- Reuses the existing Base, Sidebar and Content hierarchy. On narrow/touch
	-- screens the sidebar becomes a drawer instead of shrinking the content.
	local mobileSupport = Settings.MobileSupport ~= false
	local mobileBreakpoint = tonumber(Settings.MobileBreakpoint) or 900
	local mobileMargin = tonumber(Settings.MobileMargin) or 8
	local mobileLayout = false
	local sidebarOpen = true
	local applyingResponsive = false
	local desktopSize = base.Size
	local desktopPosition = base.Position

	local drawerDim = Instance.new("TextButton")
	drawerDim.Name = "MobileDrawerDim"
	drawerDim.Text = ""
	drawerDim.AutoButtonColor = false
	drawerDim.BackgroundColor3 = Color3.new(0, 0, 0)
	drawerDim.BackgroundTransparency = 0.35
	drawerDim.BorderSizePixel = 0
	drawerDim.Size = UDim2.fromScale(1, 1)
	drawerDim.Visible = false
	drawerDim.ZIndex = 24
	drawerDim.Parent = base

	local drawerButton = Instance.new("TextButton")
	drawerButton.Name = "MobileDrawerButton"
	drawerButton.Text = "MENU"
	drawerButton.Font = Enum.Font.GothamBold
	drawerButton.TextSize = 22
	drawerButton.TextColor3 = Color3.fromRGB(235, 235, 235)
	drawerButton.AutoButtonColor = false
	drawerButton.BackgroundColor3 = Color3.fromRGB(25, 25, 25)
	drawerButton.BackgroundTransparency = 0.05
	drawerButton.BorderSizePixel = 0
	drawerButton.Position = UDim2.fromOffset(10, 10)
	drawerButton.Size = UDim2.fromOffset(44, 44)
	drawerButton.Visible = false
	drawerButton.ZIndex = 23
	drawerButton.Parent = content
	local drawerButtonCorner = Instance.new("UICorner")
	drawerButtonCorner.CornerRadius = UDim.new(0, 10)
	drawerButtonCorner.Parent = drawerButton
	local drawerButtonStroke = Instance.new("UIStroke")
	drawerButtonStroke.Color = Color3.fromRGB(255, 255, 255)
	drawerButtonStroke.Transparency = 0.85
	drawerButtonStroke.Parent = drawerButton

	local mobileToggle = Instance.new("TextButton")
	mobileToggle.Name = "MobileMenuToggle"
	mobileToggle.Text = "NR"
	mobileToggle.Font = Enum.Font.GothamBold
	mobileToggle.TextSize = 16
	mobileToggle.TextColor3 = Color3.fromRGB(235, 235, 235)
	mobileToggle.AutoButtonColor = false
	mobileToggle.BackgroundColor3 = Color3.fromRGB(18, 18, 18)
	mobileToggle.BackgroundTransparency = 0.05
	mobileToggle.BorderSizePixel = 0
	mobileToggle.AnchorPoint = Vector2.new(0.5, 0.5)
	mobileToggle.Position = Settings.MobileButtonPosition or UDim2.fromScale(0.5, 0.5)
	...
	mobileToggleStroke.Transparency = 0.68
	mobileToggleStroke.Parent = mobileToggle

	local nrDragging = false
	local nrDragStart
	local nrStartPos
	local nrActiveInput
	local nrDragMoved = false
	local function updateNRButton(input)
		if not (nrDragging and nrDragStart and nrStartPos) then return end
		local delta = input.Position - nrDragStart
		if math.abs(delta.X) > 3 or math.abs(delta.Y) > 3 then
			nrDragMoved = true
		end
		local nextPosition = UDim2.new(
			nrStartPos.X.Scale,
			nrStartPos.X.Offset + delta.X,
			nrStartPos.Y.Scale,
			nrStartPos.Y.Offset + delta.Y
		)
		local camera = workspace.CurrentCamera
		if camera then
			local viewport = camera.ViewportSize
			local buttonSize = mobileToggle.AbsoluteSize
			local absX = (nextPosition.X.Scale * viewport.X) + nextPosition.X.Offset
			local absY = (nextPosition.Y.Scale * viewport.Y) + nextPosition.Y.Offset
			absX = math.clamp(absX, buttonSize.X * 0.5 + 4, math.max(buttonSize.X * 0.5 + 4, viewport.X - buttonSize.X * 0.5 - 4))
			absY = math.clamp(absY, buttonSize.Y * 0.5 + 4, math.max(buttonSize.Y * 0.5 + 4, viewport.Y - buttonSize.Y * 0.5 - 4))
			nextPosition = UDim2.fromOffset(absX, absY)
		end
		mobileToggle.Position = nextPosition
	end
	mobileToggle.InputBegan:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
			...
			nrDragMoved = false
			nrActiveInput = input
			nrDragStart = input.Position
			nrStartPos = mobileToggle.Position
		end
	end)
	UserInputService.InputChanged:Connect(function(input)
		if nrDragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
			if input.UserInputType == Enum.UserInputType.MouseMovement or input == nrActiveInput then
				updateNRButton(input)
			end
		end
	end)
	UserInputService.InputEnded:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.MouseButton1 or input == nrActiveInput then
			nrDragging = false
			nrActiveInput = nil
		end
	end)

	local function SetCompactSidebarVisuals(compact)
		windowControls.Visible = false
		title.Visible = not compact
		subtitle.Visible = not compact
		userInfo.Visible = true
		userAndDisplayFrame.Visible = not compact
		divider.Visible = true
		globalSettingsButtonArrow.Text = sidebarOpen and "<" or ">"
		informationHolderUIPadding.PaddingLeft = UDim.new(0, compact and 0 or 16)
		informationHolderUIPadding.PaddingRight = UDim.new(0, compact and 0 or 14)
		globalSettingsButton.AnchorPoint = Vector2.new(0.5, 0.5)
		globalSettingsButton.Position = compact and UDim2.fromScale(0.5, 0.5) or UDim2.fromScale(1, 0.5)
		globalSettingsButton.Size = UDim2.fromOffset(compact and 32 or 20, compact and 32 or 20)
		sidebarGroupUIPadding.PaddingLeft = UDim.new(0, compact and 0 or 8)
		sidebarGroupUIPadding.PaddingRight = UDim.new(0, compact and 0 or 8)
		informationGroupUIPadding.PaddingLeft = UDim.new(0, compact and 9 or 14)
		userInfoUIPadding.PaddingLeft = UDim.new(0, compact and 0 or 8)
		userInfoUIPadding.PaddingRight = UDim.new(0, compact and 0 or 8)
		tabSwitchersScrollingFrameUIListLayout.Padding = UDim.new(0, compact and 8 or 17)
		for _, item in ipairs(sidebar:GetDescendants()) do
			if item.Name == "TabSwitcherName" and item:IsA("TextLabel") then
				item.Visible = not compact
			elseif item.Name == "TabSwitcherUIPadding" and item:IsA("UIPadding") then
				item.PaddingLeft = UDim.new(0, compact and 0 or 18)
				item.PaddingRight = UDim.new(0, compact and 0 or 22)
			elseif item.Name == "TabDivider" and item:IsA("Frame") then
				item.Visible = not compact
				item.Size = compact and UDim2.fromOffset(0, 1) or UDim2.new(1, 0, 0, 1)
				item.AnchorPoint = Vector2.new(0.5, 0)
				item.Position = UDim2.fromScale(0.5, 0)
			elseif item.Name == "TabImage" and item:IsA("ImageLabel") then
				item.ImageTransparency = 0.42
			elseif item.Name == "TabSwitcherUIStroke" and item:IsA("UIStroke") then
				item.Transparency = compact and 1 or item.Transparency
			elseif item.Name == "TabSwitcherUIListLayout" and item:IsA("UIListLayout") then
				item.HorizontalAlignment = compact and Enum.HorizontalAlignment.Center or Enum.HorizontalAlignment.Left
			elseif item.Name == "TabSwitcher" and item:IsA("TextButton") then
				item.Size = compact and UDim2.fromOffset(36, 38) or UDim2.new(1, -12, 0, 40)
				if compact then item.BackgroundTransparency = 1 end
			end
		end
	end

	local function SetSidebarOpen(state)
		sidebarOpen = state == true
		local compactWidth = tonumber(Settings.MobileRailWidth) or 44
		local expandedWidth = mobileLayout and math.min(232, math.max(190, base.AbsoluteSize.X * 0.64)) or defaultSidebarWidth
		sidebar.Visible = true
		sidebar.Size = UDim2.new(0, sidebarOpen and expandedWidth or compactWidth, 1, 0)
		drawerDim.Visible = mobileLayout and sidebarOpen
		drawerButton.Text = sidebarOpen and "<" or ">"
		drawerButton.TextSize = 22
		if mobileLayout then
			content.AnchorPoint = Vector2.zero
			content.Position = UDim2.fromOffset(compactWidth, 0)
			content.Size = UDim2.new(1, -compactWidth, 1, 0)
		else
			content.AnchorPoint = Vector2.new(1, 0)
			content.Position = UDim2.fromScale(1, 0)
			content.Size = UDim2.new(0, base.AbsoluteSize.X - sidebar.Size.X.Offset, 1, 0)
		end
		drawerButton.Position = UDim2.fromOffset(8, 10)
		SetCompactSidebarVisuals(not sidebarOpen)
	end

	local function ApplyResponsiveLayout(force)
		if applyingResponsive then return end
		local camera = workspace.CurrentCamera
		if not camera then return end
		applyingResponsive = true
		local viewport = camera.ViewportSize
		local touchBreakpoint = tonumber(Settings.MobileTouchBreakpoint) or 1100
		local nextMobile = mobileSupport and (viewport.X <= mobileBreakpoint or (UserInputService.TouchEnabled and viewport.X <= touchBreakpoint))

		if nextMobile ~= mobileLayout or force then
			if nextMobile and not mobileLayout then
				desktopSize = base.Size
				desktopPosition = base.Position
			elseif not nextMobile and mobileLayout then
				base.Size = desktopSize
				base.Position = desktopPosition
			end
			mobileLayout = nextMobile
		end

		if mobileLayout then
			local width = math.max(240, viewport.X - mobileMargin * 2)
			local height = math.max(280, viewport.Y - mobileMargin * 2)
			base.AnchorPoint = Vector2.new(0.5, 0.5)
			base.Position = UDim2.fromScale(0.5, 0.5)
			base.Size = UDim2.fromOffset(width, height)
			local drawerWidth = math.min(232, math.max(190, width * 0.64))
			sidebar.Size = UDim2.new(0, drawerWidth, 1, 0)
			sidebar.Position = UDim2.fromOffset(0, 0)
			sidebar.ZIndex = 30
			content.AnchorPoint = Vector2.zero
			content.Position = UDim2.fromOffset(68, 0)
			content.Size = UDim2.new(1, -68, 1, 0)
			drawerButton.Visible = true
			mobileToggle.Visible = not windowState
			dividerInteract.Active = false
			dividerInteract.Interactable = false
			moveIcon.Visible = false
			for _, handle in pairs(resizeBorders) do handle.Visible = false end
			currentTab.Position = UDim2.new(0, 48, 0.5, 0)
			currentTab.Size = UDim2.new(1, -80, 0, 22)
			...
			drawerDim.Visible = false
			mobileToggle.Visible = false
			dividerInteract.Active = sidebarOpen
			dividerInteract.Interactable = sidebarOpen
			moveIcon.Visible = not Settings.DragStyle or Settings.DragStyle == 1
			for _, handle in pairs(resizeBorders) do handle.Visible = true end
			currentTab.Position = UDim2.fromScale(0, 0.5)
			currentTab.Size = UDim2.fromScale(0.9, 0)
			SetSidebarOpen(sidebarOpen)
		end

		maxWindowWidth = math.max(minWindowWidth, viewport.X - 20)
		maxWindowHeight = math.max(minWindowHeight, viewport.Y - 20)
		maxSidebarWidth = math.max(minSidebarWidth, base.AbsoluteSize.X - minSidebarWidth)
	...
	drawerDim.MouseButton1Click:Connect(function()
	...
	mobileToggle.MouseButton1Click:Connect(function()
		...
		mobileToggle.Text = "NR"
		mobileToggle.Visible = not windowState
		if windowState and mobileLayout then SetSidebarOpen(false) end
	end)

	local responsiveCamera = workspace.CurrentCamera
	if responsiveCamera then
		responsiveCamera:GetPropertyChangedSignal("ViewportSize"):Connect(function()
	...
	task.defer(function() ApplyResponsiveLayout(true) end)
	...
	homeButtonCallback = homeButtonCallback or nil
	...
	function WindowFunctions:SetHomeButtonCallback(callback)
		homeButtonCallback = callback
		globalSettingsButton.ImageTransparency = 1
	end

	function WindowFunctions:CreateHomeDashboard(Settings)
		Settings = Settings or {}
		local HomeFunctions = {}
		local homeConnections = {}
		local createdAt = os.clock()
		local fpsValue, fpsFrames, fpsClock = 0, 0, os.clock()
		local keyData = Settings.Key or {}

		local home = Instance.new("Frame")
		home.Name = "HomeDashboard"
		home.BackgroundTransparency = 1
		home.Position = UDim2.fromOffset(0, 63)
		home.Size = UDim2.new(1, 0, 1, -63)
		home.ClipsDescendants = true

		local scrolling = Instance.new("ScrollingFrame")
		scrolling.Name = "HomeScrolling"
		...
		scrolling.ScrollBarImageTransparency = 0.45
		scrolling.Parent = home

		local homePadding = Instance.new("UIPadding")
		homePadding.PaddingTop = UDim.new(0, 14)
		homePadding.PaddingBottom = UDim.new(0, 14)
		homePadding.PaddingLeft = UDim.new(0, 14)
		homePadding.PaddingRight = UDim.new(0, 14)
		homePadding.Parent = scrolling

		local grid = Instance.new("UIGridLayout")
		grid.Name = "HomeGrid"
		grid.CellPadding = UDim2.fromOffset(12, 12)
		grid.CellSize = UDim2.new(0.5, -6, 0, 198)
		grid.FillDirectionMaxCells = 2
		grid.SortOrder = Enum.SortOrder.LayoutOrder
		grid.Parent = scrolling

		local function UpdateHomeGrid()
			local width = math.max(0, scrolling.AbsoluteSize.X - 28)
			local useTwoColumns = width >= 430
			grid.FillDirectionMaxCells = useTwoColumns and 2 or 1
			grid.CellPadding = UDim2.fromOffset(useTwoColumns and 12 or 0, 12)
			grid.CellSize = useTwoColumns and UDim2.new(0.5, -6, 0, 198) or UDim2.new(1, 0, 0, 198)
		end
		table.insert(homeConnections, scrolling:GetPropertyChangedSignal("AbsoluteSize"):Connect(UpdateHomeGrid))
		task.defer(UpdateHomeGrid)

		local function NewText(parent, text, size, transparency, bold)
			local item = Instance.new("TextLabel")
			item.BackgroundTransparency = 1
			item.FontFace = Font.new(assets.interFont, bold and Enum.FontWeight.SemiBold or Enum.FontWeight.Medium)
			item.Text = text
		...
		local function NewCard(name, order, height)
			local card = Instance.new("Frame")
			card.Name = name
			card.LayoutOrder = order
			card.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
			card.BackgroundTransparency = 0.975
			card.BorderSizePixel = 0
			card.Size = UDim2.new(1, 0, 0, height or 210)
			card.Parent = scrolling
			local radius = Instance.new("UICorner")
			radius.CornerRadius = UDim.new(0, 9)
			radius.Parent = card
			local border = Instance.new("UIStroke")
			border.Color = Color3.fromRGB(255, 255, 255)
			border.Transparency = 0.91
			border.Parent = card
		...
		local function NewRows(card, titleText)
			local titleLabel = NewText(card, string.upper(titleText), 11, 0.42, true)
			titleLabel.Size = UDim2.new(1, 0, 0, 18)
			local rows = Instance.new("Frame")
			rows.Name = "Rows"
			rows.BackgroundTransparency = 1
			rows.Position = UDim2.fromOffset(0, 30)
			rows.Size = UDim2.new(1, 0, 1, -30)
			rows.Parent = card
		...
		local function NewInfoRow(parent, caption, initial)
			local row = Instance.new("Frame")
			row.BackgroundTransparency = 1
			row.Size = UDim2.new(1, 0, 0, 22)
			row.Parent = parent
			local captionLabel = NewText(row, caption, 11, 0.5, false)
			captionLabel.Size = UDim2.new(0.46, 0, 1, 0)
			local valueLabel = NewText(row, initial or "--", 11, 0.12, true)
		...
		local profile = NewCard("Profile", 1, 198)
		...
		local display = NewText(profile, LocalPlayer.DisplayName, 17, 0, true)
		display.Position = UDim2.fromOffset(88, 7)
		display.Size = UDim2.new(1, -88, 0, 25)
		local usernameLabel = NewText(profile, "@" .. LocalPlayer.Name, 11, 0.5, false)
		usernameLabel.Position = UDim2.fromOffset(88, 34)
		usernameLabel.Size = UDim2.new(1, -88, 0, 20)
		local accountLabel = NewText(profile, "Account age  " .. tostring(LocalPlayer.AccountAge) .. " days", 11, 0.5, false)
		accountLabel.Position = UDim2.fromOffset(88, 56)
		accountLabel.Size = UDim2.new(1, -88, 0, 18)
		local profileRows = Instance.new("Frame")
		profileRows.BackgroundTransparency = 1
		profileRows.Position = UDim2.fromOffset(0, 92)
		profileRows.Size = UDim2.new(1, 0, 1, -92)
		profileRows.Parent = profile
		local profileLayout = Instance.new("UIListLayout")
		profileLayout.Padding = UDim.new(0, 6)
		profileLayout.Parent = profileRows
		NewInfoRow(profileRows, "User ID", tostring(LocalPlayer.UserId))
		local membershipName = LocalPlayer.MembershipType == Enum.MembershipType.Premium and "Premium" or "Standard"
		local platformName = "Desktop"
		if UserInputService.TouchEnabled and UserInputService.KeyboardEnabled then
			platformName = "Tablet / Hybrid"
		elseif UserInputService.TouchEnabled then
			platformName = "Mobile"
		elseif UserInputService.GamepadEnabled and not UserInputService.KeyboardEnabled then
			platformName = "Console"
		end
		NewInfoRow(profileRows, "Membership", membershipName)
		NewInfoRow(profileRows, "Platform", platformName)
		task.spawn(function()
		...
		local serverRows = NewRows(NewCard("Server", 2, 198), "Server")
		local pingLabel = NewInfoRow(serverRows, "Ping", "Measuring...")
		local playersLabel = NewInfoRow(serverRows, "Players", "--")
		local serverAgeLabel = NewInfoRow(serverRows, "Hub uptime", "0s")
		local shortServerId = "Studio"
		if game.JobId ~= "" then shortServerId = string.sub(game.JobId, 1, 8) .. "..." .. string.sub(game.JobId, -4) end
		NewInfoRow(serverRows, "Server ID", shortServerId)
		local placeNameLabel = NewInfoRow(serverRows, "Experience", "Loading...")
		task.spawn(function()
			local ok, info = pcall(function()
				return game:GetService("MarketplaceService"):GetProductInfo(game.PlaceId)
			end)
			if ok and info and placeNameLabel.Parent then placeNameLabel.Text = tostring(info.Name or game.Name)
			else placeNameLabel.Text = game.Name ~= "" and game.Name or "Unknown" end
		end)

		local keyRows = NewRows(NewCard("AccessKey", 3, 198), "Access key")
		local keyStatusLabel = NewInfoRow(keyRows, "Status", tostring(keyData.Status or "Not configured"))
		local keyPlanLabel = NewInfoRow(keyRows, "Plan", tostring(keyData.Plan or "24-hour access"))
		local keyExpiryLabel = NewInfoRow(keyRows, "Expires (local)", "Waiting for key system")
		local keyRemainingLabel = NewInfoRow(keyRows, "Remaining", "--")
		NewInfoRow(keyRows, "Device lock", tostring(keyData.DeviceLock or "Not configured"))

		local systemRows = NewRows(NewCard("System", 4, 198), "Client")
		local fpsLabel = NewInfoRow(systemRows, "FPS", "--")
		local memoryLabel = NewInfoRow(systemRows, "Memory", "--")
		local viewportLabel = NewInfoRow(systemRows, "Viewport", "--")
		local executorName = "Unknown"
		pcall(function()
			local name, version = identifyexecutor()
			executorName = tostring(name or "Unknown")
			if version and tostring(version) ~= "" then executorName ..= " " .. tostring(version) end
		end)
		if executorName == "Unknown" then pcall(function() executorName = tostring(getexecutorname()) end) end
		NewInfoRow(systemRows, "Executor", executorName)
		NewInfoRow(systemRows, "Locale", tostring(LocalPlayer.LocaleId))
		...
		local function RefreshDashboard()
			...
			pcall(function()
				...
				ping = string.format("%.0f ms", value)
			end)
			pingLabel.Text = ping
			playersLabel.Text = string.format("%d / %d", #Players:GetPlayers(), Players.MaxPlayers)
			serverAgeLabel.Text = FormatDuration(os.clock() - createdAt)
			fpsLabel.Text = tostring(fpsValue)
			pcall(function() memoryLabel.Text = string.format("%.0f MB", stats:GetTotalMemoryUsageMb()) end)
			local camera = workspace.CurrentCamera
			if camera then viewportLabel.Text = string.format("%d x %d", camera.ViewportSize.X, camera.ViewportSize.Y) end
			local expiry = tonumber(keyData.ExpiresAt)
			if expiry then
				local remaining = expiry - os.time()
				keyRemainingLabel.Text = remaining > 0 and FormatDuration(remaining) or "Expired"
				keyExpiryLabel.Text = os.date("%Y-%m-%d %H:%M", expiry)
				keyStatusLabel.Text = remaining > 0 and tostring(keyData.Status or "Active") or "Expired"
			else
				keyRemainingLabel.Text = "--"
				keyExpiryLabel.Text = "Waiting for key system"
			end
		end

		local function ReflowHome()
			local width = scrolling.AbsoluteSize.X
			grid.CellSize = width < 620 and UDim2.new(1, -2, 0, 210) or UDim2.new(0.5, -6, 0, 210)
		end

		table.insert(homeConnections, scrolling:GetPropertyChangedSignal("AbsoluteSize"):Connect(ReflowHome))
		table.insert(homeConnections, RunService.RenderStepped:Connect(function()
			fpsFrames += 1
			local now = os.clock()
			if now - fpsClock >= 1 then
				fpsValue = math.floor(fpsFrames / (now - fpsClock) + 0.5)
				fpsFrames, fpsClock = 0, now
				RefreshDashboard()
			end
		end))

		function HomeFunctions:Select()
			if currentTabInstance then currentTabInstance.Parent = nil end
			home.Parent = content
			currentTabInstance = home
			currentTab.Text = Settings.Title or "Home"
			if mobileLayout then SetSidebarOpen(false) end
			RefreshDashboard()
		end

		function HomeFunctions:UpdateKey(newData)
			keyData = newData or {}
			keyPlanLabel.Text = tostring(keyData.Plan or "24-hour access")
			keyStatusLabel.Text = tostring(keyData.Status or "Not configured")
			RefreshDashboard()
		end

		function HomeFunctions:Refresh()
			RefreshDashboard()
		end

		function HomeFunctions:Destroy()
			for _, connection in ipairs(homeConnections) do connection:Disconnect() end
	...
	function WindowFunctions:TabGroup()
		...
		-- Available immediately, even when no tab has been created yet.
		function SectionFunctions:Divider()
			local dividerLine = Instance.new("Frame")
			dividerLine.Name = "TabDivider"
			...
			dividerLine.LayoutOrder = tabIndex
			dividerLine.Parent = sectionTabSwitchers
			tabIndex += 1
			return dividerLine
		end
		...
		function SectionFunctions:Tab(Settings)
			...
			elementsScrolling.ScrollBarImageTransparency = 1
			...
			function TabFunctions:Section(Settings)
				...
				function SectionFunctions:Dropdown(Settings, Flag)
					...
					dropdownImage.ImageTransparency = 1
			...
			function TabFunctions:InsertConfigSection(Side)
				...
				configSection:Button({
					...
					Callback = function()
						...
						local refreshedConfigs = MacLib:RefreshConfigList()
						configSelection:ClearOptions()
						configSelection:InsertOptions(refreshedConfigs)
						if autoloadSelection then
							autoloadSelection:ClearOptions()
							autoloadSelection:InsertOptions(refreshedConfigs)
						end
					end,
				})
				...
				local autoloadLabel
				autoloadSelection = configSection:Dropdown({
				...
				configSection:Button({
					...
					Callback = function()
						local name = autoloadSelection.Value or configSelection.Value
						if not name or name == "" then
							WindowFunctions:Notify({ Title = "Interface", Description = "Select an autoload config first." })
							return
						end
	...
	function WindowFunctions:Notify(Settings)
		...
		local styles = {
			...
			Confirm = function() interactable.Text = "OK" end,
			Cancel = function() interactable.Text = "X" end
		}
		...
		-- Connect only when the selected style kept the interaction button.
		if Settings.Style == "Confirm" or Settings.Style == "Cancel" then
	...
	local function ConfirmCloseMenu()
	...
	if topbarMinimize then topbarMinimize.MouseButton1Click:Connect(ToggleMenu) end
	if topbarMaximize then topbarMaximize.MouseButton1Click:Connect(function() WindowFunctions:SetState(true) end) end
	exit.MouseButton1Click:Connect(ConfirmCloseMenu)
	if topbarClose then topbarClose.MouseButton1Click:Connect(ConfirmCloseMenu) end
...
function MacLib:CreateHUB(Settings)
	Settings = Settings or {}
 
	local title    = Settings.Title    or "Noliar HUB"
	local subtitle = Settings.Subtitle or game:GetService("MarketplaceService"):GetProductInfo(game.PlaceId).Name
	local size     = Settings.Size     or UDim2.fromOffset(868, 650)
	local keybind  = Settings.Keybind  or Enum.KeyCode.RightControl
 
	-- â”€â”€ Feature Registry â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
	local FeatureRegistry = {}
	FeatureRegistry._hooks = {}
 
	function FeatureRegistry:Register(flag, applyFn)
		self._hooks[flag] = applyFn
	end
 
	function FeatureRegistry:ApplyAll(data)
		-- Step 1: sliders, dropdowns, colorpickers
		for flag, value in pairs(data) do
			local obj = MacLib.Options[flag]
			if obj then
				if obj.Class == "Slider" then
					local num = tonumber(value)
					obj:UpdateValue(num)
					if obj.Settings and obj.Settings.Callback then
						pcall(obj.Settings.Callback, num)
					end
				elseif obj.Class == "Dropdown" then
					obj:UpdateSelection(value)
					if obj.Settings and obj.Settings.Callback then
						pcall(obj.Settings.Callback, value)
					end
				elseif obj.Class == "Colorpicker" then
					if type(value) == "table" then
						local color = Color3.new(value[1], value[2], value[3])
						obj:UpdateValue(color)
						if obj.Settings and obj.Settings.Callback then
							pcall(obj.Settings.Callback, color)
						end
					end
				end
			end
		...
		for flag, value in pairs(data) do
			local obj = MacLib.Options[flag]
			if obj and obj.Class == "Toggle" then
				obj:UpdateState(value)
			end
		end
		-- Step 3: custom hooks
		for flag, applyFn in pairs(self._hooks) do
			if data[flag] ~= nil then
				pcall(applyFn, data[flag])
			end
		end
	end
 
	-- â”€â”€ Keybind persistence (separate file, never mixed with configs) â”€â”€
	local HS         = HttpService
	local KB_PATH    = MacLib.Folder .. "/_menukeybind.json"
 
	local function EnsureBaseFolder()
		if not isfolder(MacLib.Folder) then makefolder(MacLib.Folder) end
	end
	local function SaveMenuKeybind(kbName)
		EnsureBaseFolder()
		writefile(KB_PATH, HS:JSONEncode({ keybind = kbName }))
	end
	local function LoadMenuKeybind()
		EnsureBaseFolder()
		if isfile(KB_PATH) then
			local ok, data = pcall(function() return HS:JSONDecode(readfile(KB_PATH)) end)
			if ok and data and data.keybind then
				local ok2, kbEnum = pcall(function() return Enum.KeyCode[data.keybind] end)
				if ok2 and kbEnum then return kbEnum end
			end
		end
		return keybind
	end
 
	local activeKeybind = LoadMenuKeybind()
	...
	-- Home is independent from sidebar tabs; the globe always returns here.
	local Home = Window:CreateHomeDashboard({
		Title = Settings.HomeTitle or "Home",
		Key = Settings.Key or {
			Status = Settings.KeyStatus or "Not configured",
			Plan = Settings.KeyPlan or "24-hour access",
			ExpiresAt = Settings.KeyExpiresAt,
			DeviceLock = Settings.KeyDeviceLock,
		},
	})
	Window:SetHomeButtonCallback(function()
		Home:Select()
	end)
	if Settings.OnHomeCreated then
		Settings.OnHomeCreated(Home, Window)
	end
 
	-- â”€â”€ Global Settings â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
 
	-- â”€â”€ Tab Group â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
	local TabGroup = Window:TabGroup()

	-- User tabs vÃ£o ACIMA do divider
	if Settings.OnTabGroup then
		Settings.OnTabGroup(TabGroup, Window)
	end
	
	TabGroup:Divider()  -- <- divider aparece AQUI
	
	-- Sistema: Settings e Player ficam ABAIXO
	local SettingsTab = TabGroup:Tab({ Name = "Settings", Image = "rbxassetid://10734950309" })
	local PlayerTab   = TabGroup:Tab({ Name = "Player",   Image = "rbxassetid://7743875962" })
	local InterfaceSection = SettingsTab:Section({ Side = "Right" })
	InterfaceSection:Header({ Name = "Interface" })
	InterfaceSection:Toggle({
		Name = "UI Blur", Default = true,
		Callback = function(state) Window:SetAcrylicBlurState(state) end,
	}, "InterfaceBlur")
	InterfaceSection:Toggle({
		Name = "Notifications", Default = true,
		Callback = function(state) Window:SetNotificationsState(state) end,
	}, "InterfaceNotifications")
	InterfaceSection:Toggle({
		Name = "Show User Info", Default = true,
		Callback = function(state) Window:SetUserInfoState(state) end,
	}, "InterfaceUserInfo")
 
	-- â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
	--  SETTINGS TAB â€” config system
	-- â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
	local configs          = {}
	local selectedConfig   = nil
	local selectedAutoloadConfig = nil
	local ConfigDropdown
	local AutoloadDropdown
	local RefreshConfigDropdowns
	local configName       = ""
	local accountExclusive = false
	local accountAutoload  = false
	local autoloadNormal   = "none"
	local autoloadAccount  = "none"
 
	local CONFIG_FOLDER = MacLib.Folder .. "/Configs"
	...
	local function SaveMeta()
		writefile(CONFIG_FOLDER .. "/_meta.json", HS:JSONEncode({
			autoloadNormal  = autoloadNormal,
			autoloadAccount = autoloadAccount,
		}))
	end
	local function LoadMeta()
		local path = CONFIG_FOLDER .. "/_meta.json"
		if isfile(path) then
			local ok, meta = pcall(function() return HS:JSONDecode(readfile(path)) end)
			if ok and meta then
				autoloadNormal  = meta.autoloadNormal  or "none"
				autoloadAccount = meta.autoloadAccount or "none"
			end
		end
	end
	local function RefreshConfigList()
		...
		for _, path in ipairs(listfiles(CONFIG_FOLDER)) do
			...
			if name and name ~= "_meta" then table.insert(configs, name) end
		end
		return configs
	end
	local function GetCurrentConfigData()
		local data = {}
		for flag, obj in pairs(MacLib.Options) do
			if     obj.Class == "Toggle"      then data[flag] = obj:GetState()
			elseif obj.Class == "Slider"      then data[flag] = obj:GetValue()
			elseif obj.Class == "Input"       then data[flag] = obj.Text
			elseif obj.Class == "Dropdown"    then data[flag] = obj.Value
			elseif obj.Class == "Keybind"     then local b = obj:GetBind() data[flag] = b and b.Name or nil
			elseif obj.Class == "Colorpicker" then local c = obj.Color if c then data[flag] = {c.R,c.G,c.B} end
			end
		end
		return data
	end
	local function ApplyConfigData(data)
		FeatureRegistry:ApplyAll(data)
	end
	...
	-- â”€â”€ Config section â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
	local ConfigSection = SettingsTab:Section({ Side = "Left" })
	ConfigSection:Header({ Name = "Config" })
 
	ConfigSection:Input({
		Name = "Config Name", Placeholder = "Enter config name...",
		Default = "", AcceptedCharacters = "AlphaNumeric",
		Callback = function(v) configName = v end,
	}, "SettingsConfigName")
 
	ConfigSection:Toggle({
		Name = "Account Exclusive", Default = false,
		Callback = function(s) accountExclusive = s end,
	}, "SettingsAccountExclusive")
 
	ConfigSection:Toggle({
		Name = "Account Autoload", Default = false,
		Callback = function(s) accountAutoload = s end,
	}, "SettingsAccountAutoload")
 
	ConfigSection:Button({
		Name = "Create Config",
		Callback = function()
			if configName == "" then
				Window:Notify({ Title = "Settings", Description = "Type a config name first!", Lifetime = 3 }) return
			end
			local finalName = configName
			if accountExclusive then
				finalName = finalName .. "_" .. tostring(game:GetService("Players").LocalPlayer.UserId)
			end
			writefile(GetConfigPath(finalName), HS:JSONEncode(GetCurrentConfigData()))
			RefreshConfigDropdowns()
			if accountAutoload then autoloadAccount = finalName SaveMeta() end
			Window:Notify({ Title = "Config Created", Description = '"' .. finalName .. '" saved.', Lifetime = 3 })
		end,
	})
 
	-- â”€â”€ Config List section â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
	local ConfigListSection = SettingsTab:Section({ Side = "Right" })
	ConfigListSection:Header({ Name = "Config List" })
 
	ConfigDropdown = ConfigListSection:Dropdown({
		Name = "Select Config",
		Options = #configs > 0 and configs or { "nil" },
		Default = 1,
		Callback = function(v) selectedConfig = (v ~= "nil") and v or nil end,
	}, "SettingsConfigDropdown")
 

	RefreshConfigDropdowns = function()
		RefreshConfigList()
		local options = #configs > 0 and configs or { "nil" }
		if ConfigDropdown then
			ConfigDropdown:ClearOptions()
			ConfigDropdown:InsertOptions(options)
		end
		if AutoloadDropdown then
			AutoloadDropdown:ClearOptions()
			AutoloadDropdown:InsertOptions(options)
		end
	end
	...
	ConfigListSection:Button({
		...
		Callback = function()
			if not selectedConfig then
				Window:Notify({ Title = "Settings", Description = "Select a config first.", Lifetime = 3 }) return
			end
			...
			if ok and data then
				...
				Window:Notify({ Title = "Config Loaded", Description = '"' .. selectedConfig .. '" applied.', Lifetime = 3 })
			end
		end,
	})
	ConfigListSection:Button({
		...
		Callback = function()
			...
			writefile(GetConfigPath(selectedConfig), HS:JSONEncode(GetCurrentConfigData()))
			Window:Notify({ Title = "Config Overwritten", Description = '"' .. selectedConfig .. '" updated.', Lifetime = 3 })
		end,
	})
	ConfigListSection:Button({
		Name = "Delete Config",
		Callback = function()
			if not selectedConfig then return end
			if isfile(GetConfigPath(selectedConfig)) then delfile(GetConfigPath(selectedConfig)) end
			if autoloadNormal  == selectedConfig then autoloadNormal  = "none" end
			if autoloadAccount == selectedConfig then autoloadAccount = "none" end
			SaveMeta()
			selectedConfig = nil
			RefreshConfigDropdowns()
			Window:Notify({ Title = "Config Deleted", Description = "Config removed.", Lifetime = 3 })
		end,
	})
	ConfigListSection:Button({
		Name = "Refresh List",
		Callback = function()
			RefreshConfigDropdowns()
			Window:Notify({ Title = "Settings", Description = "Config list refreshed.", Lifetime = 2 })
		end,
	})
 
	-- â”€â”€ Autoload section â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
	local AutoloadSection = SettingsTab:Section({ Side = "Left" })
	AutoloadSection:Header({ Name = "Autoload" })
 
	local autoloadStatusLabel
	AutoloadDropdown = AutoloadSection:Dropdown({
		Name = "Autoload Config",
		Options = #configs > 0 and configs or { "nil" },
		Default = 1,
		Callback = function(v) selectedAutoloadConfig = (v ~= "nil") and v or nil end,
	}, "SettingsAutoloadConfigDropdown")
	local function GetAutoloadSelection()
		local value = selectedAutoloadConfig or (AutoloadDropdown and AutoloadDropdown.Value) or selectedConfig
		if value == "nil" then value = nil end
		return value
	end
	local function UpdateAutoloadLabel()
		if not autoloadStatusLabel then return end
		local n        = autoloadNormal ~= "none" and autoloadNormal or "none"
		local aDisplay = "none"
		if autoloadAccount ~= "none" then
			local uid      = tostring(game:GetService("Players").LocalPlayer.UserId)
			local baseName = autoloadAccount:gsub("_" .. uid .. "$", "")
			aDisplay       = baseName .. " (" .. game:GetService("Players").LocalPlayer.Name .. ")"
		end
		autoloadStatusLabel:UpdateName("Autoload: " .. n .. "  |  Account: " .. aDisplay)
	end
 
	AutoloadSection:Button({
		Name = "Set as Normal Autoload",
		Callback = function()
			local targetConfig = GetAutoloadSelection()
			if not targetConfig then
				Window:Notify({ Title = "Settings", Description = "Select an autoload config first.", Lifetime = 3 }) return
			end
			autoloadNormal = targetConfig
			SaveMeta() UpdateAutoloadLabel()
			Window:Notify({ Title = "Autoload Set", Description = '"' .. targetConfig .. '" loads for all accounts.', Lifetime = 3 })
		end,
	})
	AutoloadSection:Button({
		...
		Callback = function()
			...
			autoloadAccount = targetConfig
			SaveMeta() UpdateAutoloadLabel()
			Window:Notify({ Title = "Account Autoload Set", Description = '"' .. targetConfig .. '" loads for this account only.', Lifetime = 3 })
		end,
	})
	AutoloadSection:Button({
		Name = "Reset Autoloads",
		Callback = function()
			autoloadNormal = "none" autoloadAccount = "none"
			SaveMeta() UpdateAutoloadLabel()
			Window:Notify({ Title = "Autoloads Reset", Description = "No config will autoload.", Lifetime = 3 })
		end,
	})
	autoloadStatusLabel = AutoloadSection:Button({ Name = "Autoload: none  |  Account: none", Callback = function() end })
	UpdateAutoloadLabel()
 
	-- â”€â”€ Autoload on startup â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
	do
		local toLoad = nil
		if autoloadAccount ~= "none" and isfile(GetConfigPath(autoloadAccount)) then
			toLoad = autoloadAccount
		elseif autoloadNormal ~= "none" and isfile(GetConfigPath(autoloadNormal)) then
			toLoad = autoloadNormal
		end
		if toLoad then
			task.delay(1.5, function()
				local ok, data = pcall(function() return HS:JSONDecode(readfile(GetConfigPath(toLoad))) end)
				if ok and data then
					ApplyConfigData(data)
					Window:Notify({ Title = "Auto-loaded", Description = '"' .. toLoad .. '" was loaded automatically.', Lifetime = 4 })
				end
			end)
		end
	end
	...
	-- â”€â”€ Menu keybind section â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
	local KeybindSection = SettingsTab:Section({ Side = "Right" })
	KeybindSection:Header({ Name = "Menu" })
 
	KeybindSection:Keybind({
		Name    = "Toggle Keybind",
		Default = activeKeybind,
		Callback = function() end,
		onBinded = function(newBind)
			activeKeybind = newBind
			Window:SetKeybind(newBind)
			SaveMenuKeybind(newBind.Name)
			Window:Notify({ Title = title, Description = "Menu keybind set to " .. newBind.Name, Lifetime = 3 })
		end,
	}, "SettingsMenuKeybind")
	...
	local Character_        = LocalPlayer_.Character or LocalPlayer_.CharacterAdded:Wait()
	local Humanoid_         = Character_:WaitForChild("Humanoid")

	LocalPlayer_.CharacterAdded:Connect(function(char)
		Character_ = char
		Humanoid_  = char:WaitForChild("Humanoid")
	end)

	local walkSpeedEnabled = false
	local walkSpeedValue   = 16
	local PlayerSection = PlayerTab:Section({ Side = "Left" })
	PlayerSection:Header({ Name = "Walk" })
	PlayerSection:Toggle({
		Name = "WalkSpeed", Default = false,
		Callback = function(state)
			walkSpeedEnabled = state
			if Humanoid_ then Humanoid_.WalkSpeed = state and walkSpeedValue or 16 end
		end,
	}, "PlayerWalkSpeedToggle")
	PlayerSection:Slider({
		Name = "WalkSpeed Value", Minimum = 16, Maximum = 100, Default = 16,
		Precision = 0, DisplayMethod = "Value",
		Callback = function(value)
			walkSpeedValue = value
			if walkSpeedEnabled and Humanoid_ then Humanoid_.WalkSpeed = value end
		end,
	}, "PlayerWalkSpeedValue")
	FeatureRegistry:Register("PlayerWalkSpeedToggle", function(state)
		if state and Humanoid_ then Humanoid_.WalkSpeed = walkSpeedValue end
	end)

	local jumpPowerEnabled = false
	local jumpPowerValue   = 50
	local JumpSection = PlayerTab:Section({ Side = "Right" })
	JumpSection:Header({ Name = "Jump" })
	JumpSection:Toggle({
		Name = "JumpPower", Default = false,
		Callback = function(state)
			jumpPowerEnabled = state
			if Humanoid_ then
				Humanoid_.UseJumpPower = true
				Humanoid_.JumpPower = state and jumpPowerValue or 50
			end
		end,
	}, "PlayerJumpPowerToggle")
	JumpSection:Slider({
		Name = "JumpPower Value", Minimum = 50, Maximum = 300, Default = 50,
		Precision = 0, DisplayMethod = "Value",
		Callback = function(value)
			jumpPowerValue = value
			if jumpPowerEnabled and Humanoid_ then
				Humanoid_.UseJumpPower = true
				Humanoid_.JumpPower = value
			end
		end,
	}, "PlayerJumpPowerValue")
	FeatureRegistry:Register("PlayerJumpPowerToggle", function(state)
		if state and Humanoid_ then
			Humanoid_.UseJumpPower = true
			Humanoid_.JumpPower = jumpPowerValue
		end
	end)
	...
	local FlySection = PlayerTab:Section({ Side = "Left" })
	FlySection:Header({ Name = "Fly" })

	local function EnableFly()
		if flyActive then return end
		flyActive = true
		local char     = LocalPlayer_.Character
		local rootPart = char and char:FindFirstChild("HumanoidRootPart")
		if not rootPart then flyActive = false return end
		...
		flyConnection = RunService_.RenderStepped:Connect(function()
			local camera = workspace.CurrentCamera
			local cf, vel = camera.CFrame, Vector3.zero
			if UserInputService_:IsKeyDown(Enum.KeyCode.W)         then vel += cf.LookVector end
			if UserInputService_:IsKeyDown(Enum.KeyCode.S)         then vel -= cf.LookVector end
			if UserInputService_:IsKeyDown(Enum.KeyCode.A)         then vel -= cf.RightVector end
			if UserInputService_:IsKeyDown(Enum.KeyCode.D)         then vel += cf.RightVector end
			if UserInputService_:IsKeyDown(Enum.KeyCode.Space)     then vel += Vector3.new(0,1,0) end
			if UserInputService_:IsKeyDown(Enum.KeyCode.LeftShift) then vel -= Vector3.new(0,1,0) end
			if flyBodyVel  then flyBodyVel.Velocity = vel * flySpeed end
	...
	local function DisableFly()
		...
		local char     = LocalPlayer_.Character
		local rootPart = char and char:FindFirstChild("HumanoidRootPart")
		if flyBodyVel  then flyBodyVel.Velocity = Vector3.zero flyBodyVel.MaxForce = Vector3.zero flyBodyVel:Destroy() flyBodyVel = nil end
		if flyBodyGyro then flyBodyGyro.MaxTorque = Vector3.zero flyBodyGyro:Destroy() flyBodyGyro = nil end
		task.defer(function()
	...
	LocalPlayer_.CharacterAdded:Connect(function(char)
		Character_ = char
		Humanoid_  = char:WaitForChild("Humanoid")
		if flyEnabled then task.wait(0.1) DisableFly() EnableFly() end
	end)
	...
	FlySection:Keybind({
		Name = "Fly Keybind", Default = Enum.KeyCode.F3,
		Callback = function()
			MacLib.Options["PlayerFlyToggle"]:UpdateState(not flyEnabled)
		end,
	}, "PlayerFlyKeybind")
	FlySection:Slider({
		Name = "Fly Speed", Minimum = 0, Maximum = 300, Default = 50,
		Precision = 0, DisplayMethod = "Value",
		Callback = function(value) flySpeed = value end,
	}, "PlayerFlySpeed")
	FeatureRegistry:Register("PlayerFlyToggle", function(state)
		DisableFly()
		if state then EnableFly() end
	end)

	local MiscSection = PlayerTab:Section({ Side = "Right" })
	MiscSection:Header({ Name = "Misc" })
	...
	local function SetAntiAFK(state)
		...
		antiAFKConnection = LocalPlayer_.Idled:Connect(function()
			-- VirtualUser prevents the idle disconnect without moving the character.
			pcall(function()
				local camera = workspace.CurrentCamera
				local center = camera and camera.ViewportSize / 2 or Vector2.new(1, 1)
				VirtualUser:CaptureController()
				VirtualUser:Button2Down(center, camera and camera.CFrame or CFrame.new())
				task.wait(0.05)
				VirtualUser:Button2Up(center, camera and camera.CFrame or CFrame.new())
			end)
		end)
	end
	MiscSection:Toggle({
	...
	local fpsOriginal = nil
	local fpsInstanceOriginals = {}

	local function RememberProperty(instance, property)
		fpsInstanceOriginals[instance] = fpsInstanceOriginals[instance] or {}
		if fpsInstanceOriginals[instance][property] == nil then
			local ok, value = pcall(function() return instance[property] end)
			if ok then fpsInstanceOriginals[instance][property] = value end
		end
	end

	local function SetTrackedProperty(instance, property, value)
		RememberProperty(instance, property)
		pcall(function() instance[property] = value end)
	end

	local function OptimizeInstance(instance)
		if instance:IsA("PostEffect") or instance:IsA("ParticleEmitter") or instance:IsA("Trail")
			or instance:IsA("Beam") or instance:IsA("Smoke") or instance:IsA("Fire") or instance:IsA("Sparkles")
		then
	...
	local function DisconnectFPSWatchers()
		for _, connection in ipairs(fpsConnections) do connection:Disconnect() end
		table.clear(fpsConnections)
	end

	local function RestoreFPSBoost()
		DisconnectFPSWatchers()
		for instance, properties in pairs(fpsInstanceOriginals) do
		...
		table.clear(fpsInstanceOriginals)
		if fpsOriginal then
			pcall(function() settings().Rendering.QualityLevel = fpsOriginal.QualityLevel end)
			pcall(function() Lighting.GlobalShadows = fpsOriginal.GlobalShadows end)
			pcall(function() Lighting.EnvironmentDiffuseScale = fpsOriginal.EnvironmentDiffuseScale end)
			pcall(function() Lighting.EnvironmentSpecularScale = fpsOriginal.EnvironmentSpecularScale end)
			local terrain = workspace:FindFirstChildOfClass("Terrain")
			if terrain then
				pcall(function() terrain.Decoration = fpsOriginal.TerrainDecoration end)
				pcall(function() terrain.WaterWaveSize = fpsOriginal.WaterWaveSize end)
				pcall(function() terrain.WaterWaveSpeed = fpsOriginal.WaterWaveSpeed end)
				pcall(function() terrain.WaterReflectance = fpsOriginal.WaterReflectance end)
			end
		end
		fpsOriginal = nil
	...
	local function SetFPSBoost(state)
		state = state == true
		if state == fpsBoostEnabled then return end
		if not state then RestoreFPSBoost() return end
		...
		for _, instance in ipairs(Lighting:GetDescendants()) do OptimizeInstance(instance) end
		for _, instance in ipairs(workspace:GetDescendants()) do OptimizeInstance(instance) end
		table.insert(fpsConnections, Lighting.DescendantAdded:Connect(function(instance)
			if fpsBoostEnabled then task.defer(OptimizeInstance, instance) end
		end))
		table.insert(fpsConnections, workspace.DescendantAdded:Connect(function(instance)
			if fpsBoostEnabled then task.defer(OptimizeInstance, instance) end
		end))
	end

	MiscSection:Toggle({
		Name = "FPS Boost", Default = false,
		Callback = SetFPSBoost,
	}, "PlayerFPSBoost")
	MiscSection:Toggle({
		Name = "Auto Hide UI", Default = false,
		Callback = function(state)
			local SG = game:GetService("StarterGui")
			for _, item in ipairs({ Enum.CoreGuiType.Chat, Enum.CoreGuiType.PlayerList,
				Enum.CoreGuiType.Backpack, Enum.CoreGuiType.Health, Enum.CoreGuiType.EmotesMenu }) do
				pcall(function() SG:SetCoreGuiEnabled(item, not state) end)
			end
		end,
	}, "PlayerAutoHideUI")
	...
	-- â”€â”€ Final setup â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
	...
	return {
		...
		ApplyConfigData = ApplyConfigData,
	}
end
 
 --  USAGE â€” in your game script (NOT inside the lib):
--
...
--  local TabGroup        = HUB.TabGroup       -- add new tabs below the divider
--  local PlayerTab       = HUB.PlayerTab      -- add sections here
